													...
													cl_mem screen_buffer)
{
	...
	err |= clSetKernelArg(*kernel, 3, sizeof(cl_mem),
			&rt->gpu_mem->cl_counter_buffer);
	err |= clSetKernelArg(*kernel, 4, sizeof(cl_mem),
			&rt->gpu_mem->cl_light_buffer);
	err |= clSetKernelArg(*kernel, 5, sizeof(cl_mem),
			&rt->gpu_mem->cl_obj_buffer);
	...
	err |= clSetKernelArg(*kernel, 9, sizeof(cl_mem),
			&rt->gpu_mem->cl_texture_w);
	err |= clSetKernelArg(*kernel, 10, sizeof(cl_mem),
			&rt->gpu_mem->cl_texture_h);
	err |= clSetKernelArg(*kernel, 11, sizeof(cl_mem),
			&rt->gpu_mem->cl_prev_texture_size);
	return (err);
}

void		render(t_rt *rt, cl_mem screen_buffer, size_t global_size[2],
														size_t local_size[2])
{
	cl_mem		cam_buffer;
	cl_kernel	*kernel;
...
void		gauss_blur(t_rt *rt, cl_mem screen_buffer, size_t global_size[2],
														size_t local_size[2])
{
	cl_kernel	*kernel;
	cl_int		err;
	...
	err = gauss_blur_gpu_init(rt, kernel, screen_buffer);
	...
	err = clEnqueueReadBuffer(*rt->cl->queue, rt->gpu_mem->cl_img_buffer,
		CL_TRUE, 0, WIDTH * HEIGHT * sizeof(int), rt->img.data, 0, NULL, NULL);
	cl_error_handler("Couldn't read the gauss_blur buffer", err);
}

void		post_processing(t_rt *rt, cl_mem screen_buffer,
							size_t global_size[2], size_t local_size[2])
{
	...
	err |= clSetKernelArg(*kernel, 2, sizeof(cl_mem),
			&screen_buffer);
	cl_error_handler("Couldn't create a kernel post_processing argument",
			err);
	err = clEnqueueNDRangeKernel(*rt->cl->queue, *kernel, 2, NULL, global_size,
			local_size, 0, NULL, NULL);
